...
static int __get_index(set_t *set, const char *key, uint64_t hash, uint64_t *index);
static int __assign_node(set_t *set, const char *key, uint64_t hash, uint64_t index);
static void __free_index(set_t *set, uint64_t index);
static int __set_contains(set_t *set, const char *key, uint64_t hash);
static int __set_add(set_t *set, const char *key, uint64_t hash);
static void __relayout_nodes(set_t *set, uint64_t start, short end_on_null);


int set_init_alt(set_t *set, uint64_t num_els, set_hash_function hash) {
    set->nodes = (set_node**) malloc(num_els * sizeof(set_node*));
    if (set->nodes == NULL) {
...
static int __set_add(set_t *set, const char *key, uint64_t hash) {
    ...
    if ((float)set->used_nodes / (float)set->number_nodes > MAX_FULLNESS_PERCENT) {
        uint64_t num_els = set->number_nodes * 2; // we want to double each time
        set_node** tmp = (set_node**)realloc(set->nodes, num_els * sizeof(set_node*));
...
static int __assign_node(set_t *set, const char *key, uint64_t hash, uint64_t index) {
    size_t len = strlen(key);
    set->nodes[index] = (set_node*)malloc(sizeof(set_node));
